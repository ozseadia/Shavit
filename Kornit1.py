...
def main (QT,C,Customer,file_path_C):
    ...
    print(List)
    ws.cell(1,17).value='יתרה לאספקה'
    ws.cell(1,18).value='מספר הזמנה שביט'
    ws.cell(1,19).value='מק"ט'
    ws.cell(1,16).value='הפרש בימים'
    ws.cell(1,15).value='תאריך אספקה QT'
    for i in range(len(L)):
        #ind=[]
        element=str(L[L.keys()[4]][i])+str(L[L.keys()[3]][i])
        print(element)
        ...
        try:
            ...
            if ind!=[]:
                print(ind)
                ws.cell(i+2,15).value=QT[QT.keys()[10]][ind] # תאריך אספקה
                ws.cell(i+2,15).number_format = 'DD/MM/YYYY'
                temp=np.busday_count(str(today.date()),str((QT[QT.keys()[10]][ind]).date()),weekmask=[1,1,1,1,1,0,1])
                ws.cell(i+2,16).value=temp
                if temp>0:
                    for j in range(1,22):ws.cell(i+2,j).fill=greenfill
                else:
                    for j in range(1,22):ws.cell(i+2,j).fill=redfill
                ws.cell(i+2,16).value=QT[QT.keys()[11]][ind] # סטאטוס
                ws.cell(i+2,17).value=QT[QT.keys()[9]][ind] # יתרה לאספקה
                ws.cell(i+2,18).value=QT[QT.keys()[2]][ind] #מספר הזמנה שביט
                ws.cell(i+2,19).value=QT[QT.keys()[4]][ind] #מקט
                    #ws.cell(i,20).value=QT[QT.keys()[11]][ind] # תאריך אספקה שביט
            else:
                for j in range(1,22):ws.cell(i+2,j).fill=yellowfill
        except:
            0
